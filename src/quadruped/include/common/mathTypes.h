...
// 16x1 Vector
using Vec16 = typename Eigen::Matrix<double, 16, 1>;
...
inline Mat4 vec16ToVec44(Vec16 vec16){
    Mat4 mat4;
    for(int i(0); i < 4; ++i){
        mat4.col(i) = vec16.segment(3*i, 4);
    }
    return mat4;
}

inline Vec16 vec44ToVec16(Mat4 mat4){
    Vec16 vec16;
    for(int i(0); i < 4; ++i){
        vec16.segment(3*i, 4) = mat4.col(i);
    }
    return vec16;
}
