...
* @param[in]:   hcan:    指向CAN_HandleTypeDef结构的指针
* @param[in]:   motor:   指向motor_t结构的指针，包含电机相关信息和控制参数  n
* @retval:     	void
...
// 函数功能：解析DM4340电机反馈数据包
// 输入参数：motor - 关节电机数据结构体指针
void dm4340_fbdata_motor(motor_t *motor, uint8_t *rx_data, int i)
{ 
    // 解析原始字节数据
    uint8_t pos_tmp = rx_data[2+3*i]; 
    uint8_t vel_tmp = rx_data[3+3*i] ;                 
    uint8_t tor_tmp = rx_data[4+3*i];     

    // 添加浮点转换（需在头文件定义参数范围）
    motor->ctrl.pos_set = set_motor_zero_offset(uint_to_float(pos_tmp, P_MIN2, P_MAX2, 8) , i);  // 16位精度
    motor->ctrl.vel_set = uint_to_float(vel_tmp, V_MIN2, V_MAX2, 8);   // 12位精度
    motor->ctrl.tor_set = uint_to_float(tor_tmp, T_MIN2, T_MAX2, 8);   // 12位精度
...
// 函数功能：解析DM4340电机反馈数据包
// 输入参数：motor - 关节电机数据结构体指针
//          rx_data - 原始数据缓冲区（11字节，索引0-10）
void dm4340_fbdata_set(motor_t *motor, uint8_t *rx_data)
{ 
    // 解析原始字节数据
    uint16_t kp_tmp = ((rx_data[1] & 0xF0) << 4) | rx_data[0];      // 组合12位KP值
    uint16_t kd_tmp = (rx_data[2] << 4) | (rx_data[1] & 0x0F);     // 组合12位KD值

    // 添加浮点转换（需在头文件定义参数范围）
    motor->ctrl.kp_set = uint_to_float(kp_tmp, KP_MIN2, KP_MAX2, 12);   // 修正为kp_set
    motor->ctrl.kd_set = uint_to_float(kd_tmp, KD_MIN2, KD_MAX2, 12);   // 修正为kd_set
}

// 新增零点偏移设置函数
uint8_t set_motor_zero_offset(float pos_tmp, int i){
    if(i == 0) return pos_tmp + 1.0f;
	else if(i == 1)return pos_tmp - 1.0f;
	else if(i == 2)return pos_tmp + 3.0f;
	else if(i == 3)return pos_tmp + 0.15f;
	else if(i == 4)return pos_tmp - 1.0f;
	else if(i == 5)return pos_tmp + 2.0f;
	else if(i == 6)return pos_tmp + 0.0f;
	else if(i == 7)return pos_tmp + 1.40f;
	else if(i == 8)return pos_tmp + 2.4f;
	else if(i == 9)return pos_tmp - 1.8f;
	else if(i == 10)return pos_tmp + 1.6f;
	else if(i == 11)return pos_tmp + 0.7f;
	else if(i == 12)return pos_tmp + 0.0f;	
	else if(i == 13)return pos_tmp + 0.0f;
}