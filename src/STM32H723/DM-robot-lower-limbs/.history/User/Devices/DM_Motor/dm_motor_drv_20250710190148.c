...
void dm4340_fbdata_motor(motor_t *motor, uint8_t *rx_data)
{ 
    // 解析原始字节数据
    uint16_t pos_tmp = (rx_data[0] << 1) | ((rx_data[1] >> 7) & 0x01);  // 组合9位位置数据
    uint8_t vel_tmp = rx_data[1] & 0x7F;                                // 获取7位速度数据
    uint8_t tor_tmp = rx_data[2];                                       // 8位力矩数据
