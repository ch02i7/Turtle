...
{ 
    ...
    // 添加浮点转换（需在头文件定义参数范围）
    motor->ctrl.pos_set = uint_to_float(pos_tmp, P_MIN2, P_MAX2, 8);  // 16位精度
...
// 新增零点偏移设置函数
void set_motor_zero_offset(uint8_t pos_tmp, int i)
{
    motor->para.pos_offset = offset;
    // 可选：添加参数保存逻辑（需要实现非易失存储）
    // save_motor_data(motor->id, OFFSET_REG_ADDR);
}